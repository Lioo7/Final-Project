...
class Test_calculator(unittest.TestCase):
    ...
    def test_get_voter_count_by_gender(self):
        ...
        self.assertEqual(
            Calculator.get_voter_count_by_gender(self.db_connector), [0, 0]
        )  # [male,female]

        self.cursor.execute("UPDATE Users SET allowed_to_vote = 0 WHERE user_id < 50")
        self.assertEqual(
            Calculator.get_voter_count_by_gender(self.db_connector), [0, 50]
        )  # [male,female]

        self.cursor.execute("UPDATE Users SET allowed_to_vote = 0 WHERE user_id = 99")
        self.assertEqual(
            Calculator.get_voter_count_by_gender(self.db_connector), [1, 50]
        )  # [male,female]
    ...
    def test_get_voter_count_by_age(self):
        ...
        self.assertEqual(
            Calculator.get_voter_count_by_age(self.db_connector), [0, 0, 0, 0, 0, 0]
        )

        self.cursor.execute("UPDATE Users SET allowed_to_vote = 0")
        self.assertEqual(
            Calculator.get_voter_count_by_age(self.db_connector),
            [10, 15, 5, 20, 30, 20],
        )
