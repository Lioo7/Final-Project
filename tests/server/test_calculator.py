...
class Test_calculator(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS Users (id INTEGER PRIMARY KEY, first_name, last_name, birth_date , mail,
    ...
    def setUp(self):
        for i in range(0,100):
            self.cursor.execute(f'''INSERT INTO Users (id, first_name, last_name, birth_date, mail, password, gender, is_admin,
    ...
    def test_get_voter_count(self):
        ...
        self.cursor.execute("UPDATE Users SET allowed_to_vote = 0 WHERE id < 50")
        self.assertEqual(Calculator.get_voter_count(self.db_connector),50)
        
        self.cursor.execute("UPDATE Users SET allowed_to_vote = 0 WHERE id = 99")
        self.assertEqual(Calculator.get_voter_count(self.db_connector),51)
        
    
    def test_get_voter_count_by_gender(self):
        
        self.cursor.execute("UPDATE Users SET gender = 'female' WHERE id < 50")
    ...
    def test_get_voter_count_by_age(self):
        ...
        self.cursor.execute("UPDATE Users SET birth_date = '" + eighteen_years_ago.strftime("%d/%m/%Y") + "' WHERE id BETWEEN 0 AND 9")
        self.cursor.execute("UPDATE Users SET birth_date = '" + Twentysix_years_ago.strftime("%d/%m/%Y")+"' WHERE id BETWEEN 10 AND 24")
        self.cursor.execute("UPDATE Users SET birth_date = '" + Thirtysix_years_ago.strftime("%d/%m/%Y")+"' WHERE id BETWEEN 25 AND 29")
        self.cursor.execute("UPDATE Users SET birth_date = '" + fourtysix_years_ago.strftime("%d/%m/%Y")+"' WHERE id BETWEEN 30 AND 49")
        self.cursor.execute("UPDATE Users SET birth_date = '" + fiftysix_years_ago.strftime("%d/%m/%Y")+"' WHERE id BETWEEN 50 AND 79")
        self.cursor.execute("UPDATE Users SET birth_date = '" + sixtysix_years_ago.strftime("%d/%m/%Y")+"' WHERE id BETWEEN 80 AND 100")
