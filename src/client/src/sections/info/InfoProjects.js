...
// const knowledge =
//   'מערכת ההשכלה הגבוהה בישראל כוללת מוסדות המוכרים על ידי המועצה להשכלה גבוהה, שהסמיכה אותם להעניק תארים אקדמיים. תקצוב המוסדות נעשה באמצעות הוועדה לתכנון ולתקצוב (ותת) ועל ידי משרד החינוך (התומך בחלק מהמכללות לחינוך). מערכת ההשכלה הגבוהה כוללת גם מוסדות אשר אין בתקציבם השתתפות מתקציב המדינה (מוסדות חוץ-תקציביים). המוסדות להשכלה גבוהה בישראל נחלקים לשלוש קבוצות: אוניברסיטאות העוסקות במחקר ובהוראה ומעניקות שלושה תארים אקדמיים: תואר ראשון, תואר שני ותואר שלישי.האוניברסיטה הפתוחה המבוססת על לימוד מרחוק ועל לימודים חלקיים ומעניקה תואר אקדמי ראשון ושני.מכללות שעיקר עיסוקן בהוראה ומעניקות תואר ראשון ושני של למודים אקדמיים כלליים או לימודים מקצועיים.מוסדות אקדמיים להכשרת עובדי הוראה.';
// const security =
//   'שיעור תקציב הבטחון מהתמג גבוה בישראל מאשר במדינות אירופה וארהב, אולם הוא נמצא בתוואי יורד בשנים האחרונות, וכך גם האחוז שהוא מהווה מההוצאה הממשלתית. החל משנת 2017 הועבר כל מענק הסיוע הבטחוני האמריקאי כהוצאה המותנית כהכנסה, מה שמתבטא בצמצום תקציב הנטו של המשרד. באירועים חריגים בעבר נוספו לתקציב המשרד תוספות בגין עלויות לחימה (כמו, למשל, במלחמת לבנון השניה או מבצע עופרת יצוקה).';
// const internalSecurity =
//   'המשרד לביטחון פנים הוא גוף מטה משולב האחראי לאכיפת החוק, לשמירת הסדר הציבורי ולפעילות בתחומים השונים של ביטחון הפנים במדינת ישראל.המשרד פועל, בין היתר, באמצעות משטרת ישראל, שירות בתי הסוהר, הרשות הלאומית לכבאות והצלה, הרשות להגנה על עדים, הרשות הלאומית למלחמה בסמים ובאלכוהול וגופים נוספים.';
// const fund =
...
// const ConstructionAndHousing =
//   'משרד הבינוי והשיכון פועל בשני מישורים בתחום הדיור:היצע ? פעילות המשרד מכוונת לתכנון ולפיתוח קרקע לצורך שיווקה לבנייה למגורים, במטרה ליצור תנאים להרחבת היצע הדיור בדרך שתאפשר מתן מענה לביקוש. הפעילות כוללת מגוו?ן רחב של מרכיבים בשרשרת הייצור של דירה: איתור הקרקע, תכנונה ושיווקה; עידוד עיבוי הבנייה במרכזי הערים תוך ניצול מערכת התשתית הקיימת; שדרוג התשתיות הקיימות; ורישום הזכויות במקרקעין. לכל אחד מהמרכיבים השפעה על היקף הפעילות בענף.ביקוש ? הסיוע בדיור מכ?וון לתמיכה בזכאים מקרב אוכלוסיות חלשות ומתמקד בשלושה מסלולים עיקריים: מתן משכנתאות מסובסדות, השתתפות בתשלומי שכר דירה והקצאת פתרונות דיור בשיכון הציבורי.';
// const energy =
...
// const primeMinister =
//   'משרד ראש הממשלה כולל את היחידות הגופים והתחומים הכפופים ישירות לאחריות ראש הממשלה ויחידות וגופים עצמאיים בקביעת תכניות העבודה שלהם וניהול התקציב.היחידות והגופים הכפופים ישירות לאחריות ראש הממשלה: יועצים לראש הממשלה; האגף לממשל וחברה; האגף לתכנון, תיאום ופנים; אגף הכלכלה והתשתיות; המועצה הלאומית לכלכלה; המועצות להנצחת נשיאים וראשי ממשלה ומרכזי ההנצחה למנחם בגין וליצחק רבין; מטה ההסברה הלאומי; מערך הגיור; מטה התקשוב הממשלתי ועוד.היחידות והגופים העצמאיים בקביעת תכניות העבודה שלהם ובניהול תקציבם: המשרד לירושלים ומורשת; המשרד לנושאים אסטרטגיים והסברה; המשרד לענייני מודיעין; המשרד לשוויון חברתי; הלשכה המרכזית לסטטיסטיקה; מנהלת תנופה ? יישום תכנית ההתנתקות; נציבות שירות המדינה; לשכת הקשר נתיב; המשרד לפיתוח הפריפריה, הנגב והגליל והמשרד לשיתוף פעולה אזורי.נוסף על יחידות אלו, יחידות המטה לביטחון לאומי, לשכת הפרסום הממשלתי והמדפיס הממשלתי הן יחידות הכפופות למשרד ראש הממשלה, אולם הן מתוקצבות בסעיפים תקציביים נפרדים.';
...
// //   19: EconomyAndIndustry,
// //   20: primeMinister,
// //   21: religiousServices,
// //   22: laws,
// //   23: curator,
// //   24: outside,
// //   25: localAuthority,
...
// // const infoProjects = {
// //     'חינוך': education,
// //     'מדע, תרבות וספורט': sport,
// //     'העברות לביטוח לאומי': insurance,
// //     'בריאות': health,
// //     'רווחה': welfare,
// //     'השכלה גבוהה': knowledge,
// //     'ביטחון': security,
// //     'ביטחון פנים': internalSecurity,
// //     'קרן/ביטוח לאומי': fund,
// //     'ריבית': interest,
// //     'תחבורה': transport,
// //     'בינוי ושיכון': ConstructionAndHousing,
// //     'אנרגיה': energy,
// //     'משק המים': waterSector,
// //     'תקשורת': communication,
// //     'תיירות': tourism,
// //     'חקלאות': agriculture,
// //     'הגנת הסביבה': environmentalProtection,
// //     'כלכלה ותעשיה': EconomyAndIndustry,
// //     'ראש הממשלה': primeMinister,
// //     'שירותי דת': religiousServices,
// //     'משפטים': laws,
// //     'אוצר': curator,
// //     'חוץ': outside,
// //     'פנים ושלטון מקומי': localAuthority,
// //     'גמלאות': retirement,
// //     'רזרבה': reserve,
// //     'הוצאות שונות': variousExpenses
// // }
...
//     id: 8,
//     title: 'ביטחון פנים',
//     info: internalSecurity,
//   },
//   {
//     id: 9,
//     title: 'קרן/ביטוח לאומי',
//     info: fund,
//   },
//   {
//     id: 10,
...
//     id: 18,
//     title8: 'הגנת הסביבה',
//     info: environmentalProtection,
//   },
//   {
//     id: 19,
//     title: 'כלכלה ותעשיה',
//     info: EconomyAndIndustry,
//   },
//   {
//     id: 20,
//     title: 'ראש הממשלה',
